...
import { updateTrip } from '@/app/lib/actions3'; // Import the function for updating a trip

interface Props {
  trip: any; // Define the type of the trip prop
  drivers: string[]; // Define the type of the drivers prop
  vehicles: string[]; // Define the type of the vehicles prop
}

export default function Form({ trip, drivers, vehicles }: Props) {
  const [isLoading, setIsLoading] = useState(false);
  const [selectedDriver, setSelectedDriver] = useState(trip.driver_email);
  const [selectedVehicle, setSelectedVehicle] = useState(trip.vehicle_number);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await updateTrip(trip._id, formData); // Use the function for updating a trip
      // If updateTrip does not throw an error, display success toast
      toast.success('Trip updated successfully!');
    } catch (error) {
      // If updateTrip throws an error, display error toast
