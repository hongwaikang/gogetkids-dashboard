...
import { parseClassesJSON, parseClassesCSVToJSON, insertClassesFromJSON } from '@/app/lib/bulkUpload';
...
const BulkImportForm = () => {
    ...
    if (jsonFileInputRef.current && jsonFileInputRef.current.files) { // Update this line
      ...
      reader.onload = async (event) => {
        if (event.target && event.target.result) {
          ...
          try {
            // Parse the JSON data
            const parsedClasses = await parseClassesJSON(jsonData);
            console.log('Parsed classes:', parsedClasses);

            // Insert classes into the database
            await insertClassesFromJSON(parsedClasses);
            console.log('Classes inserted successfully');
            toast.success('Classes inserted successfully!');
          } catch (error) {
      ...
      reader.onload = async (event) => {
        if (event.target && event.target.result) {
          ...
          try {
            ...
            const parsedClasses = await parseClassesCSVToJSON(csvData);
            console.log('Parsed classes from CSV:', parsedClasses);
  ...
  "school_name": "School 123"
        ...
        <div>
          ...
          <pre className="mt-2 bg-gray-100 p-2 rounded-md overflow-auto">class_name,class_level,teacherid,school_name</pre>
