...
import { updateSchedule } from '@/app/lib/actions'; // Import the function for updating a schedule

interface Props {
  schedule: any; // Define the type of the schedule prop
  students: string[]; // Define the type of the students prop
}

export default function Form({ schedule, students }: Props) {
  const [isLoading, setIsLoading] = useState(false);
  const [selectedStudent, setSelectedStudent] = useState(schedule.studentid);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await updateSchedule(schedule._id, formData); // Use the function for updating a schedule
      // If updateSchedule does not throw an error, display success toast
      toast.success('Schedule updated successfully!');
    } catch (error) {
      // If updateSchedule throws an error, display error toast
      ...
      <div className="rounded-md bg-gray-50 p-4 md:p-6">
        ...
        <div className="mb-4">
          ...
          <select
            id="transport_type"
            name="transport_type"
            defaultValue={schedule.transport_type}
        ...
        <div className="mb-4">
          ...
          <div className="relative mt-2 rounded-md">
            <input
              ...
              name="pickup_time"
              type="time"
              defaultValue={schedule.pickup_time}
              className="peer block w-full rounded-md border border-gray-200 py-2 text-sm outline-2 placeholder:text-gray-500"
              required
            />
          </div>
        ...
        <div className="mb-4">
          ...
          <div className="relative mt-2 rounded-md">
            <input
              ...
              name="dismissal_time"
              type="time"
              defaultValue={schedule.dismissal_time}
              className="peer block w-full rounded-md border border-gray-200 py-2 text-sm outline-2 placeholder:text-gray-500"
              required
            />
          </div>
